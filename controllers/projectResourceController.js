...
const trackResourceUsage = async (req, res) => {
  try {
    const { project_name, resource_name, used_quantity, allocated_by } =
      req.body;
    ...
    if (!user) {
      return res
        .status(404)
        .json({ message: `ไม่พบผู้ใช้งานชื่อ '${allocated_by}'` });
    }
    ...
    const updatedResource = await Resource.update(
      { quantity: resource.quantity - used_quantity },
      { where: { resource_name } }
    );
    ...
    if (!project) {
      return res
        .status(404)
        .json({ message: `ไม่พบโครงการชื่อ '${project_name}'` });
    }
...
const getUsageByProject = async (req, res) => {
  try {
    ...
    if (!usage || usage.length === 0) {
      return res
        .status(404)
        .json({ message: "No resource usage found for this project" });
    }
...
const getAllProjectResources = async (req, res) => {
  try {
    ...
    if (!projectResources || projectResources.length === 0) {
      return res
        .status(404)
        .json({ message: "ไม่พบรายการการใช้ทรัพยากรในโครงการ" });
    }
